...
const isCyclic = (x: number) =>  {
    ...
    //if (xPrimeFactors!.length > 2) return false
    if (xPrimeFactors!.length === 1 && xPrimeFactors![0] !== 2) return true
    if (xPrimeFactors!.length === 2 && xPrimeFactors![0] === 2 && (x / 2) % 2 !== 0) return true
    return false
}
